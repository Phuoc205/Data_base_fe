...
function Keyboard(props) {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch('http://localhost:3000/products/keyboard');
            ...
            {selectedProduct ? (
              <div className="product-detail">
                ...
                <div className="main-products-detail">
                    ...
                    <p>Switch: {selectedProduct.SWITCH}</p>
                    <p>Model: {selectedProduct.MODEL}</p>
                    <p>Size: {selectedProduct.KEYBOARD_SIZE}</p>
                    <p>Material of keycap: {selectedProduct.KEYCAP_MATERIAL}</p>
                    <p>Weight: {selectedProduct.KEYBOARD_WEIGHT}</p>
                  </div>
